...
Install the necessary packages with `pip install -r requirements.txt`. Run `code/main.py` to successfully generate all the results. This is what will run in the background: 
  ...
  6. Streamlit: Run `streamlit run dashboard.py` to generate the dashboard for a visual representation of our results. It calls functions from `code/data_analysis.py` that analyse tables for the dashboard. It will open as a new tab in your browser. 
