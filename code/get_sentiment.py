...
def get_site_sentiment(site):
    ...
    site_results = pd.concat(
        [sentiments, sentiments_count, sentiments_pct, upvotes_pct, votes_by_sentiment],
        axis=1,
    )
    site_results.columns = [
        "Sentiment",
        "Sentiment Count",
        "Sentiment Pct",
        "Upvotes Pct",
    ...
    if site == "Reddit":
        site_results.to_sql(
            "reddit_site-results", con=engine, if_exists="replace", index=False
        )

    if site == "Twitter":
        site_results.to_sql(
            "twitter_site-results", con=engine, if_exists="replace", index=False
        )

    # Site level comparison
    post_sentiment = (
        comments.groupby(["reddit_post_id"])["score_compound"].mean().reset_index()
    )
    post_sentiment.columns = ["Post ID", rtitle]

    if site == "Reddit":
        comments.to_sql(
            "temp_red_comm_sentiment", con=engine, if_exists="replace", index=False
        )

        reddit_sentiment_sql = """
            UPDATE reddit_comments AS f
            SET score_neg = t.score_neg,
            score_neu = t.score_neu,
            score_pos = t.score_pos,
            score_compound = t.score_compound,
            score = t.score
    ...
    if site == "Twitter":
        comments.to_sql(
            "temp_twit_comm_sentiment", con=engine, if_exists="replace", index=False
        )
...
def get_sentiment():
    ...
    post_comparison = reddit_post_results.merge(
        twitter_post_results, on="Post ID", how="left"
    )
    post_comparison["Diff"] = (
        post_comparison["Twitter Score"] - post_comparison["Reddit Score"]
    )
    post_comparison["Same"] = np.where(abs(post_comparison["Diff"]) <= 0.2, True, False)

    post_comparison.to_sql(
